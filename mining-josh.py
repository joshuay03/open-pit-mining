...
class Mine(search.Problem):
    ...
    def payoff(self, state):
        ...
        if state.ndim == 1:
            return np.array([np.sum(self._underground[i][:state[i]], dtype=float, axis=0) for i in range(len(state))])
        else:
            return np.array([[np.sum(self._underground[j][i][:state[j][i]], dtype=float, axis=0) for i in range(len(state[j]))] for j in range(len(state))])
